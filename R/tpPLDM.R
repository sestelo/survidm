tpPLDM <-
  function(object, s, t, conf = FALSE, n.boot = 199, conf.level = 0.95,
	   cluster = FALSE, ncores = NULL)
  {
    ...
    t0 <- t[t <= max(object[[1]]$time1)]
    t1 <- t[t <= max(object[[1]]$Stime)]
    n0 <- length(t0)
    n1 <- length(t1)
    ...
    kmw0 <- PKMW(object[[1]]$time1[p0], object[[1]]$event1[p0])
    kmw1 <- PKMW(object[[1]]$Stime[p0], object[[1]]$event[p0])
    if(state1 > 0) kmw2 <- PKMW(object[[1]]$Stime[p1], object[[1]]$event[p1])
	...
	kmw0 <- PKMW(ndata$time1[p0], ndata$event1[p0])
	kmw1 <- PKMW(ndata$Stime[p0], ndata$event[p0])
	if(state1 > 0) kmw2 <- PKMW(ndata$Stime[p1], ndata$event[p1])
