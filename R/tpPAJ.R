tpPAJ <- function(object, s, conf = FALSE, conf.level = 0.95)
{
  if (missing(object))
...
#  if (!inherits(object, "survIDM")) stop("'object' must be of class 'survIDM'")
  ...
  res <- TPmsm::transPAJ(object=TPobj, s=s, t=Inf, conf=conf, conf.level=conf.level)
  #res <- list()
  ...
  if(conf == TRUE) {
    ...
    res <- list(
      # states information:
      s = s, t = max(res$time), states = c("1","2","3"), ns = 3, tr.states = c("1","2"),
      conf.type = "bootstrap",
      # event times:
      times = res$time,
      probs = probs, all.probs = all.probs,
      # posible transitions:
      p.trans = ptrans,
      conf = conf)
